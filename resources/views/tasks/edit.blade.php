        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required
                    class="mt-1 block w-full bg-[var(--task-canvas)] border border-[var(--task-border)] rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[var(--task-accent)] focus:border-[var(--task-accent)] sm:text-sm">{{ old('description', $task->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
